...
public class SocialGroupResource {
    ...
    //@Smita: Simple Rest call to access social group by extID
    @RequestMapping(value = "/{extId}", method = RequestMethod.GET, produces = "application/xml")
    public ResponseEntity<? extends Serializable> getSocialByExtId(@PathVariable String extId) 
    {
    	SocialGroup socialGruop = new SocialGroup();
    	try {

    		socialGruop = socialGroupService.findSocialGroupById(extId, null);
    	...
    	return new ResponseEntity<SocialGroup>(ShallowCopier.copySocialGroup(socialGruop), HttpStatus.OK);

    }

    //@Smita: Simple Rest call to access all social groups

    @RequestMapping(method = RequestMethod.GET, produces = "application/xml")
    @ResponseBody
    public SocialGroups getAllSocialGropus() 
    {

    	List<SocialGroup> socialGroups = socialGroupService.getAllSocialGroups();
    	List<SocialGroup> copies = new ArrayList<SocialGroup>(socialGroups.size());
    	for (SocialGroup sg: socialGroups) {
    		SocialGroup copy = ShallowCopier.copySocialGroup(sg);
    		copies.add(copy);
    	}

    	SocialGroups socialGroupall = new SocialGroups();
    	socialGroupall.setSocialGroups(copies);

    	return socialGroupall;
    }
